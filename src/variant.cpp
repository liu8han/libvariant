...
{
    string result = "";
    char * pOut = new char [((length + 2) / 3) * 4 + 1];
    memset (pOut, 0x00, ((length + 2) / 3) * 4 + 1);
    int32_t outlen = a2base64 ((char *) pBuffer, length, pOut, ((length + 2) / 3) * 4 + 1);
    if (outlen > 0)
        result = string (pOut, outlen);
    delete [] pOut;
...
string unb64 (uint8_t *pBuffer, uint32_t length)
{
    ...
    int32_t outlen = base64dec ((char *)pBuffer, length, pOut, length);
    string result(pOut, outlen);
    delete[] pOut;
    return result;
}
